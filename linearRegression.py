...
Name: Andrew Abbott
Username: aabbott
Computes the linear regression line of a given data set using matrix algebra
Usage: python linearRegression.py
...
def gradient(b_current, m_current, points, learningRate):
    ...
    x = points[:, 0]
    y = points[:, 1]
    b_gradient = sum(-(2/N) * (y - ((m_current * x) + b_current)))
    m_gradient = sum(-(2/N) * x * (y - ((m_current * x) + b_current)))
...
def plot_results(points,m,b):
    ...
    plt.scatter(x_training,y_training,color='b')
